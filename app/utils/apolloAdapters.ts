...
type ServiceFunction<TResult, TVariables = unknown> = (args?: TVariables) => Promise<TResult>;
...
export function createQueryAdapter<TResult, TVariables>(
  ...
  const fetchData = async (args?: TVariables): Promise<TResult | null> => {
    loading.value = true;
    error.value = null;
    try {
